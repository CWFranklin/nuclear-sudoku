$(function () {
    ...
    socket.on('pReset', function (data) {
        ...
        var $lives = $('#lives');
        $lives.empty();
        for (var i = 0; i < 3; i++) $lives.append('<li>');
    });

    socket.on('pUpdateTargets', function (targets) {
        var $targets = $('<select><option>Select a target</option></select>');
        for (var i in targets)
            if (i !== me) $targets.append('<option value="' + i + '">' + targets[i] + '</option>');

        $('#targets, .launch').html($targets.html());
    });

    socket.on('kill', function (data) {
        $('#gameover').html('You were killed by <strong>' + data.killer + '</strong>'
            ...
            +'<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="https://platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>').show();
    });
    ...
    socket.on('pWin', function () {
        $('#gameover').html('Congratulations, you survived nuclear sudoku!'
            +'<small>Please wait for the next round to begin</small>'
